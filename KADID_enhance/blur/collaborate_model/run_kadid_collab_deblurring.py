...
# ✅ StackFeaturePooling, UNetConvBlock, CollaborativeUNet (base_channels=128 적용)
class StackFeaturePooling(nn.Module):
    def __init__(self, pooling_type='avg'):
        super().__init__()
        self.pooling_type = pooling_type
...
def visualize_comparison(original, restored, index, save_dir="results"):
    ...
    to_pil = transforms.ToPILImage()
    fig, axes = plt.subplots(1, 2, figsize=(8, 4))
    axes[0].imshow(to_pil(original.cpu().clamp(0, 1)))
    axes[0].set_title("Original Blur")
    axes[0].axis("off")
...
def run_kadid_collab_deblurring(csv_path, img_dir):
    ...
    model.load_state_dict(torch.load(
        "C:/Users/IIPL02/Desktop/NEW/KADID_enhance/blur/collaborate_model/weights/collab_unet_blur_ref.pth",
        map_location=device
    ))
    model.eval()
    ...
    for _, row in df.iterrows():
        filename = row['dist_img']
        if not any(code in filename for code in blur_codes):
            continue
        ...
        with torch.no_grad():
            ...
            restored_patches = model(patches)
            visualize_comparison(patches[0].cpu(), restored_patches[0].cpu(), count)
            count += 1
