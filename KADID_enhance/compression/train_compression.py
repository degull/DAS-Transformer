...
class VGGPerceptualLoss(nn.Module):
    def __init__(self):
        super(VGGPerceptualLoss, self).__init__()
        vgg = vgg16(weights="IMAGENET1K_V1").features[:16]  # conv3_3까지
        for param in vgg.parameters():
            param.requires_grad = False
        self.vgg = vgg.eval()
        self.criterion = nn.L1Loss()
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (img, ref, distortion_class) in enumerate(dataloader):
        ...
        img = img.to(device)
        ref = ref.to(device)
        distortion_class = distortion_class.to(device)

        optimizer.zero_grad()

        # ✅ JPEG (10) vs JPEG2000 (9) 복원 분기
        restored = model(img, class_id=distortion_class)
    ...
    # ✅ 매 epoch마다 모델 저장
    model_path = os.path.join(checkpoint_dir, f"epoch_{epoch+1:02d}.pth")
